...
//enlève brick de brick_ptr dans bricl_space
...
//bouge brick_ptrà l'endroit le plus bas où il peut être et change cela dans brick_space
...
int can_remove(struct brick *brick_ptr, struct brick *removed_brick_arr[], int *removed_brick_num, struct brick**** brick_space){
	...
	for(int i = 0; i < cord_num; ++i){
		...
		if(above_ptr != NULL){
			...
			if(old_hight != new_hight){
				...
				*removed_brick_num += 1;
				++total_bricks_moved;
				remove_brick(above_ptr, brick_space);
			}
		}
	}
...
int main (){
    ...
    int scan_num;
	int remove_num;
	struct brick *can_remove_arr[2000];
	...
	for(int i = 0; i < brick_num; ++i){
		...
		while(scan_num != remove_num){
			total_brick_bougees += can_remove(can_remove_arr[scan_num], can_remove_arr, &remove_num, brick_space);
			++scan_num;
		}
		for(int j = 0; j < remove_num; ++j)
			place_brick(can_remove_arr[j], brick_space);
	}
