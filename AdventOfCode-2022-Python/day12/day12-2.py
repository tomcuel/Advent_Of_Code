...
def possible_neighbors(i,j) :
    ...
    for di,dj in [(1,0),(-1,0),(0,1),(0,-1)] : 
        ...
        if 0<=ii<n and 0<=jj<m and heigth(map[ii][jj]) >= heigth(map[i][j])-1: #changing this because we go backwards
...
# but we're revversing it, because we want to end on a "a" or on "S" which both are the lowest value of the map : 0
...
while True : 
    ...
    if heigth(map[i][j]) == height_of_start: 
...
# the result is 29 for the test.txt file as expected