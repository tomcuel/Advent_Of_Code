...
int main(){
    ...
    start_beam.direction=trouver_direction(start_beam.direction, 0, 0, tableau_miroir, nline, tailleline);
    // printf("\ntest de la fonction trouver_direction : %d\n",start_beam.direction);


    beam_logic(start_beam, memo, tableau_miroir, power_tiles, nline, tailleline);

    printf("Tableau des lignes illuminées :\n");
    for (int i=0; i<nline; i++){
        for (int j=0; j<tailleline; j++){
            printf("%c",power_tiles[i][j]);
        }
        printf("\n");
    }
    printf("\n");
    ...
    // compter le nombre de # dans la map résultat 
...
// on trouve bien 46 pour le fichier test.txt