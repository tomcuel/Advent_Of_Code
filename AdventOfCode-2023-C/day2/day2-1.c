...
// Fonction pour trouver le numéro de game
int nmgame(char *ligne){
    int k=0;
    int n=strlen(ligne);
    if (ligne[5] >= 48 && ligne[5] <= 57 && ligne[5+1] == ':'){
        k=ligne[5]-48;
    }

    if (ligne[5] >= 48 && ligne[5] <= 57 && ligne[5+1] >= 48 && ligne[5+1] <= 57 && ligne[5+2] == ':'){
        k=(ligne[5]-48)*10+ligne[5+1]-48;
    }

    if (ligne[5] >= 48 && ligne[5] <= 57 && ligne[5+1] >= 48 && ligne[5+1] <= 57 && ligne[5+2] >= 48 && ligne[5+2] <= 57 && ligne[5+3] == ':'){
        k=(ligne[5]-48)*100+(ligne[5+1]-48)*10+ligne[5+2]-48;
    }


    affichage1(ligne,n);
    printf("numéro de game : %d\n\n",k);
    return(k);
}
...
// on regarde si les chiffres permettent d'être en dessous de la configuration initiale pour le jeu 
int ligneokey(char* ligne, struct game initial){
    ...
    free(positionsbleu);
    free(positionsvert);
    // on va donner k=1, si la ligne est ok
    int k=0;
    if (lignenb.rouge <= initial.rouge && lignenb.bleu <= initial.bleu && lignenb.vert <= initial.vert ){
        k=1;
    }
...
// doit retourner 8 pour le test pour test.txt