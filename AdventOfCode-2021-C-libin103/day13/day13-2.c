...
int main (){
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
        ...
        // printf("height : %d / width : %d\n", atoi(str_height), atoi(str_width));
    }
    ...
    // printing the answer :
    for (int k = 0; k < number_of_commands; k++){
        printf("The number of dots that are visible after the %d step is : %d\n", k+1, integer_set_size(&new_dots[k]));
    }

    int numbers_of_dots[1000];
    for (int i = 0; i < 1000; i++){
        numbers_of_dots[i] = 0;
    }
    for (int k = 0; k < 1000; k++){
        if (integer_set_is_member(&new_dots[number_of_commands-1], k)){
            numbers_of_dots[k] = k;
        }
    }
...
// we found the right answer (password) but there is the need to read it the other way around 
// for my input, it's FPEKBEJL, but it just printed it the other way around