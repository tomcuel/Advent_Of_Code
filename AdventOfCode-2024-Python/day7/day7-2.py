...
def can_operate(number_to_match, numbers, combo) : 
    ...
    for i in range(len(numbers)-1) : 
        if combo[i] == "+" : 
            ...
            number_created = int(str(number_created) + str(numbers[i+1]))
    return number_created == number_to_match
    
# we sum the results of the lines operation
# still done in brute force
answer = 0 
for line in lines : 
    # we test every combination of + and * for the numbers in the line
    # in this part the | is added as a possibility to concatenate the numbers and need to be taken into account
...
# print the answer
print("Answer:", answer)
# we found 11387 for the test7.txt file as expected