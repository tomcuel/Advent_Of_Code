...
# doing the BFS algorithm to get the shortest path from the start to the end, then to the start and back to the end
# we then start from the start point, using heap to get the shortest path

# The first false represent if we have hit the end point going from the first start point
# the second one if we have hit the start point going from the first end point
heap=[(0,start,False,False)]
visited=set()
...
while len(heap) > 0 : 
    ...
    time, location, has_hit_end, has_hit_start= point
    row, col = location

    # looking if there is nothing wrong : hitting the start point before the end point
    assert not (has_hit_start and not has_hit_end)

    # looking if the point is not in a blizzard state
    # time % period to get the state of the grid at this time, the states of the grid are making loop of positions
    assert not occupied(location, states_blizzard[time % period])

    # stopping condition
    if location == end:
        if has_hit_start and has_hit_end:
            print("\nThe shortest path is time is : ",time,"\n")
            break
        # noting that we've hit the end point
        has_hit_end = True

    if location == start:
        if has_hit_end  :
            # noting that we've hit the start point only if we've already hit the end point once before since this is the order
            has_hit_start = True
    ...
    for drow, dcol in (directions+[[0,0]]):
        ...
        new_point= (time + 1, new_location, has_hit_end, has_hit_start)
...
# we found 54 as expected with the test.txt file