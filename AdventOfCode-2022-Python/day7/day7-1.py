...
sum = 0
# we sum the size of the directories that are less than 100000
# we can sum them many times but we are going to take them as many times as we need to
# for example if we have a directory of size 1000 and it is in 3 different directories, we are going to take it 3 times
for abspath in directory_size:
    if dfs(abspath) <= 70000000:
        sum += dfs(abspath)

# we print the result
print("the result is : ", sum,"\n")

# we find 95437 as expected with the test.txt file