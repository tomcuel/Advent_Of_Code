...
// Fonction pour trouver la différence entre deux tableaux en diminuant la taille du tableau de 1
...
// Fonction pour trouver le chiffre précédent selon le tableau de différences
int chiffreprecedent(int* T,int p){
    int res=0;
    int k=p;
    int i=0;
    ...
    for (int q=1; q<p-1;q++){
        Tab[p-1-q][q+1]=Tab[p-1-q][0]-Tab[p-q][q];
    }

    res=Tab[0][0]-Tab[1][p-1];
...
// on trouve bien 2 pour le test.txt