...
int is_valid(struct chemin** chemin_array, int nline, int ligne, int colonne, int direction){
    ...
    if (chemin_array[ligne][colonne].est_visité == 1 || chemin_array[ligne][colonne].type == '#'){
        return(0);
    }
...
// on trouve bien 154 pour test.txt 
// programme très long quand même, surement pas assez optimisé, pour l'input, mais toujours assez rapide pour test.txt sinon 