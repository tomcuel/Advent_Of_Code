...
// Function to see in what type of part we are in the file
int get_new_source(current_source old){
    switch (old) {
    case SEEDS:
        return SOIL;
    case SOIL:
...
// Function to map the source to the destination
void map_src_to_dest(uint32_t dest, uint32_t source, uint32_t range, uint32_t *old, uint32_t *new){
    if (*old >= source && (*old < (source + range))) {
        *new = *old - source + dest;
    }
}

// Function to map the sources to the old
void map_sources_to_old(struct path paths[NSEEDS], current_source current_source){
    int i;
