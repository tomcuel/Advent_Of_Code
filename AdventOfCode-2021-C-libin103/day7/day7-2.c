...
int main (){
    ...
    for (int i=0; i<max_number; i++){
        ...
        for (int j=0; j<nb_numbers; j++){
            dist=abs(numbers[j]-i);
            fuel_cost=dist*(dist+1)/2; // analysing the fuel cost datas from the puzzle
            temp+=fuel_cost;
        }
        // updating the result
...
// I found 168 as expected for the result of the test.txt file