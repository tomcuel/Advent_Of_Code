...
for line in instructions : 
    ...
    temp_list = []
    # we will pop the letter on top of the stack and add it to a list for the number of iterations needed, to store what has be pop in which order 
    for _ in range(number):
        pop = stacks[source].pop()
        temp_list.append(pop)
    # we reverse the list to don't change the order of what we take from the source stack
    temp_list.reverse()
    # we will add the letters from the list to the destination stack, so that it will be in the right order
    for i in range(number):
        stacks[destination].append(temp_list[i])
    

tops = [stack[-1] for stack in stacks]
print("".join(tops))

# the output is : MCD as it was expected with the test file test.txt
